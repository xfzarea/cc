...
public class SqlJob {
	...
	public void execute() {
		...
		if (jobs != null && jobs.size() > 0) {
			...
			for (Map<String, Object> job : jobs) {
				...
				if((double)job.get("totalAward")>(double)job.get("award")&&job.get("transaction_id")!=null){
					...
					if (state == 1) {// 红包处于正常状态
						if (dataDao.checkCash(jobId) == null) {// 防止出现二次提交
		...
		dues = jobDao.pushTimeDue();// 订阅提醒
		if (dues != null && dues.size() > 0) {
			for (Map<String, Object> due : dues) {
				forms = dataDao.getTimerFormId((int) due.get("id"));
				jobDao.updateTakePush((int) due.get("id"));// 修改 takePush
				if (forms != null && forms.size() > 0) {
					for (Map<String, Object> form : forms) {
						String msg = Util.getMsg(form, due, 3);
						userService.sendMsg(msg);
